import cv2
import numpy as np
from .utils import lane_colors, OffsetType
...
class PerspectiveTransformation(object):
    ...
    def __init__(self, img_size=(1280, 960), src=None, dst=None):
    ...
    def transformToBirdViewPoints(self, points):
        ...
        points_array = []
        if (len(points)) :
            for x, y in points: points_array.append([x, y])
            if (len(points_array)) :
    ...
    def calcCurveAndOffset(self, img, left_lanes, right_lanes):
        ...
        if (len(left_lanes) and len(right_lanes)) :
            ...
            ym_per_pix = 30/self.img_size[1] # meters per pixel in y dimension
            xm_per_pix = 3.5/self.img_size[0] # meters per pixel in x dimension
            y_eval = np.max(ploty)
            ...
            lane_width = np.absolute(leftx[self.img_size[1]-1] - rightx[self.img_size[1]-1])

            lane_xm_per_pix = 3.5 / lane_width
            veh_pos = ((leftx[self.img_size[1]-1] + rightx[self.img_size[1]-1])  / 2.)

            cen_pos = (img.shape[1]/ 2.)
            img = cv2.arrowedLine(img, (int(veh_pos), int(y_eval)), (int(veh_pos), int(img.shape[1]/3)), (255, 255, 255), 5, 0, 0 , 0.2)
            img = cv2.arrowedLine(img, (int(cen_pos), int(y_eval)), (int(cen_pos), int(img.shape[0]/1.3)), (150, 255, 150), 10, 0, 0 , 0.5)
        ...
        img = cv2.putText(img,  f'Offset: {abs(distance_from_center):.1f} m'  , (20, 80), cv2.FONT_HERSHEY_SIMPLEX, 3, (0, 0, 255), 5)
        img = cv2.putText(img,  f'Curvature : {curvature:.1f} m', (20, 180), cv2.FONT_HERSHEY_SIMPLEX, 3, (0, 0, 255), 5)
        return (curvature_direction, curvature), distance_from_center, img
    ...
    def DrawTransformFrontalViewAreaOnBirdView(self, warped):
        vector = np.vectorize(np.int_)
        cv2.line(warped, vector(self.dst[0]), vector(self.dst[1]), (0, 0, 255), 5)
        cv2.line(warped, vector(self.dst[1]), vector(self.dst[2]), (0, 0, 255), 5)
        cv2.line(warped, vector(self.dst[2]), vector(self.dst[3]), (0, 0, 255), 5)
        cv2.line(warped, vector(self.dst[3]), vector(self.dst[0]), (0, 0, 255), 5)
        return warped