...
class TaskConditions(object):
	...
	def UpdateCollisionStatus(self, vehicle_distance, lane_area, distance_thres=10) : 
		"""
		...
		if (vehicle_distance != None) :
			...
			if len(self.vehicle_collision_record) > 3: 
				...
				if (avg_vehicle_collision <= distance_thres): 						self.collision_msg = CollisionType.WARNING
				elif (distance_thres < avg_vehicle_collision <= 3*distance_thres): self.collision_msg = CollisionType.PROMPT
				else: 																self.collision_msg = CollisionType.NORMAL
		else:
			if (lane_area): self.collision_msg = CollisionType.NORMAL
			else: self.collision_msg = CollisionType.UNKNOWN